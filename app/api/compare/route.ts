...
export async function POST(req: Request) {
  ...
  const openaiText = body.openaiText || "";
  const geminiText = body.geminiText || ""; 
  const judge = body.judge || "mock";
...
`;
  ...
  if (useMock) {
    const openaiSafe = openaiText || "";
    const geminiSafe = geminiText || "";

    const best =
      openaiSafe.length > geminiSafe.length
        ? "A"
        : "B";
